...
class P5_Amazon_Dataset(Dataset):
	def __init__(self, all_tasks, task_list, tokenizer, args, sample_numbers, mode='train', split='toys',
		...
		elif 'val' in self.mode: # elif self.mode == 'val':
		...
		if task_name == 'rating':
			...
			if task_template['id'] == '1-1':
				...
				if rand_prob > 0.5:
					...
					overall_idx = random.randint(0, len(
						overall_candidates) - 1)  # random choose the overall index for overall_candidates
					source_text = task_template['source'].format(self.user2id[rating_datum['reviewerID']],
																 self.item2id[rating_datum['asin']],
			...
			if task_template['id'] == '3-1':
				...
				source_text = task_template['source'].format(exp_datum['feature'], int(exp_datum['overall']), user_desc,
															 self.item2id[exp_datum['asin']])
				target_text = task_template['target'].format(exp_datum['explanation'])
			elif task_template['id'] == '3-13':
				if 'reviewerName' in exp_datum:
					user_desc = exp_datum['reviewerName']
				else:
					user_desc = exp_datum['reviewerID']
				source_text = task_template['source'].format(int(exp_datum['overall']), user_desc,
															 self.item2id[exp_datum['asin']])
				target_text = task_template['target'].format(exp_datum['explanation'])
			else:
				raise NotImplementedError
			...
			if self.mode == 'train':
				...
				target_idx = random.randint(0, len(
					target_candidates) - 1)  # random choose the target index for target_candidates
	...
	def collate_fn(self, batch):
		...
		for i, entry in enumerate(batch):
			...
			if 'target_length' in entry:
				target_length.append(len(entry['target_text']))
...
class P5_Yelp_Dataset(Dataset):
	def __init__(self, all_tasks, task_list, tokenizer, args, sample_numbers, mode='train', split='yelp',
				 rating_augment=False, sample_type='random'):
		self.all_tasks = all_tasks
		self.task_list = task_list
		self.tokenizer = tokenizer
